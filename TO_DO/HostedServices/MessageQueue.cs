...
public class MessageQueue
{
    ...
    public async Task<Transaction?> Dequeue()
    {
        var scope = _serviceProvider.CreateScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<ToDoDbContext>();
        var transaction = await dbContext.Transactions.Where(x => x.Status == TransactionStatus.Created).OrderBy(t=>t.Id).FirstOrDefaultAsync();
    ...
    public async Task Acimolage(int id)
    {
        var scope = _serviceProvider.CreateScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<ToDoDbContext>();
        var transaction = await dbContext.Transactions.FirstOrDefaultAsync(t => t.Status == TransactionStatus.Processing && t.Id == id);
        if(transaction == null)
        {
            return;
        }
